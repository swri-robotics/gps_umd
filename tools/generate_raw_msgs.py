...
messages live in ``gps_msgs``, a pure interface package that must never gain a
libgps dependency. Regenerating is a
...
# GPSFix/GPSStatus.
#
# They used to have a package of their own. That was worth its cost when the
# generated set ran to roughly 900 messages -- flattening the report arms into
# the root message left one message per API pair, eleven in total, which builds
# in seconds and no longer justifies a second package for every consumer to
# depend on.
#
# gps_msgs therefore holds both hand-written and generated messages. Everything
...
#
# Emitted here rather than checked in by hand because orphans() owns every
# MESSAGE_PREFIX-named file in msg/, and this one is named GPSDJson: checked in
# by hand it would be reported as stale by the next run.
JSON_MESSAGE = """\
...
# gps.h macros that name legal *values* for a published field, rather than
# bits of the `set` report mask. Without these a subscriber gets a bare int and
# has to go read gps.h to interpret it.
#
# Each is re-prefixed after the field it describes. That is forced, not
# cosmetic: `STATUS_GPS`, `MODE_3D`, `SEEN_GPS` and `ANT_OK` are all gps.h
# macros, so a message constant of the same name is destroyed by the
# preprocessor in any translation unit that includes gps.h -- the same hazard
# that turned `LATLON_SET` into `SET_LATLON`.
#
...
def value_constants(src: str) -> List[Tuple[str, int, str, bool]]:
    ...
    instead, so the `(1llu<<N)` form is excluded here by requiring a literal.
    """
    out = []
    for macro, value in re.findall(
        ...
        for gpsd_prefix, msg_prefix, _field, is_bits in VALUE_CONSTANT_FAMILIES:
            if macro.startswith(gpsd_prefix):
                out.append((msg_prefix + macro[len(gpsd_prefix):],
                            int(value, 0), macro, is_bits))
                break
    return sorted(out)
...
def emit_msg(model: Model, name: str, rev: str,
    ...
    for const_name, value in constants:
        ...
        # else stays decimal.
        #
        # rosidl rejects expressions in constants -- `1 << 4` does not parse,
        # only a literal does -- so GPSd's own `(1llu<<4)` spelling cannot be
        # carried over. Hex is accepted and keeps the bit legible where
        # decimal does not: 0x0000000000200000 says which bit is set,
        # 2097152 does not.
        n = int(value, 0)
...
def emit_value_asserts(model: Model, values) -> List[str]:
    ...
    would reach a subscriber as a plausible number. Each is `#ifdef`-guarded
    because a build may use an earlier rev of the same API pair where the
    macro does not exist yet -- ANT_* arrives during API 14, ANT_PWR_* during
    API 16, and STATUS_GPS replaced STATUS_FIX during API 12.
    """
    if not values:
        return []
    root = versioned(MESSAGE_PREFIX + "Raw", model.pair)
...
def emit_parser(model: Model, rev: str, constants=()) -> str:
    ...
    if constants:
        ...
        out += emit_value_asserts(model, value_constants(model.src))
...
def orphans(files: Dict[str, str], output_root: str) -> List[str]:
    ...
    """
    owned = {
        # gps_msgs holds hand-written messages too, so ownership here is the
        # MESSAGE_PREFIX and not the whole directory: claiming the directory
        # would propose deleting GPSFix.msg and GPSStatus.msg. The same prefix
