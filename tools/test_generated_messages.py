...
def message_fields(pair, stem):
    """Field names of gps_msgs/msg/<stem><major>v<minor>.msg, or None."""
    text = generated().get(f"gps_msgs/msg/{stem}{pair[0]}v{pair[1]}.msg")
    if text is None:
        return None
...
def message_constants(pair, stem="GPSDRaw"):
    text = generated().get(f"gps_msgs/msg/{stem}{pair[0]}v{pair[1]}.msg") or ""
...
class GeneratedParserCode(unittest.TestCase):
    ...
    def test_includes_match_rosidl_header_names(self):
        for pair in ALL_PAIRS:
            source = self.parser_source(pair)
            suffix = f"{pair[0]}v{pair[1]}"
            self.assertIn(f"#include <gps_msgs/msg/gpsd_raw{suffix}.hpp>", source)
            self.assertIn(f"#include <gps_msgs/msg/gpsd_fix{suffix}.hpp>", source)
